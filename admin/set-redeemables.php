        ...
        <section class="section dashboard">
            ...
            <div class="container-fluid px-4">
                    ...
                    <h4 class="text-dark fw-bold lead">
                       Set Redeemables
                     <a  onclick="add_redeemable()" class="btn text-light float-end" style="margin-left: 10px; background-color: #00c5ce;"><i class="fa fa-add fa-fw" ></i>
                         Add Redeemable
                    </a>
                    </h4>   
                </div>
              <!-- content -->
              <div class="card-body mt-4" id="machine_slot" style="height: 600px; overflow-y: auto;">
<?php
    $redeemables_select =$db->query("SELECT * FROM redeemables_table ORDER BY item_id ASC");
    if($redeemables_select->num_rows > 0)
    {
        while($rows = mysqli_fetch_array($redeemables_select))
        {
...
?>  
    <div class="schedules">
        <div class="schedule-info">
            <h4><i class="fas fa-box"></i> <?php echo $item_name?></h4>
            <h6><i class="fa fa-list"></i> Description: <?php echo $description ?></h6>
            <h6><i class="fa fa-list"></i> Budz Points: <?php echo $redeemable_point ?></h6>
        </div>
        <div class="schedule-choices">
            <a class="btn btn-success" onclick="edit_redeemable('<?php echo $rows['item_id'] ?>')"><i class="fas fa-edit"></i></a>
            <a class="btn btn-danger" onclick="delete_alert(<?php echo $item_id ?>)"><i class="fas fa-trash-can"></i></a>
        </div>
    </div>
...
?>
    ...
    <!-- End Footer -->
     <!-- add modal -->
<div class="modal fade" id="add_redeemable_modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-md">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="exampleModalLabel">Add Rdeemable</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <form action="code/code.php" method="POST" id="add_redeemable_form">
        <div class="form-group mt-2" >
        <label>Name</label>
        <input id="add_item_name" name="add_item_name" class="form-control" value="" required/>
        </div>
        <div class="form-group mt-2" >
        <label>Description</label>
        <input id="add_description" name="add_description" class="form-control" value=""/>
        </div>
        <div class="form-group mt-2">
        <label>Points</label>
        <input id="add_redeemable_point" name="add_redeemable_point" class="form-control" value="" required/>
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
        <button type="submit" class="btn btn-primary" name="add_redeemable_btn">Add</button>
      </div>
    </div>
  </div>
</div>
...
<!-- edit modal -->
<div class="modal fade" id="edit_redeemable_modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-md">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <div class="modal-body">
        <form method="POST" id="edit_redeemable_form">
        <input id="item_id" name="item_id" class="form-control" type="hidden" value=""/>
        <div class="form-group mt-2" >
        <label>Name</label>
        <input id="item_name" name="item_name" class="form-control" value="" required/>
        </div>
        <div class="form-group mt-2" >
        <label>Description</label>
        ...
        <div class="form-group mt-2">
        <label>Points</label>
        <input id="redeemable_point" name="redeemable_point" class="form-control" value="" required/>
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
        <button type="submit" class="btn btn-primary" name="edit_redeem_btn">Save changes</button>
      </div>
    </div>
  </div>
</div>
...
<script type="text/javascript">
     function edit_redeemable(id)
          ...
          $.ajax({
            ...
            success: function (response)
             {  
                ...
                $('#redeemable_point').val(data.point);
             },
          });
          $('#edit_redeemable_modal').modal("show");
        }
          $('#edit_redeemable_form').submit(function(e)
          {
            action = 12;
            e.preventDefault();
            item_id = $('#item_id').val();
            ...
            redeemable_point = $('#redeemable_point').val();
            $.ajax({
                url: "code/function.php",
                type: "POST",
                datatype: 'json',
            ...
            {
                console.log("error");
                Swal.fire("Something weng wrong!", "", "error");
                setTimeout(() => {
                document.location.reload();
                }, 1000);
            }); 
          });
            function delete_redeemable(id)
        {
            action = '11';
            item_id = id;
            $.ajax({
        ...
        function add_redeemable()
        {
            $('#add_redeemable_modal').modal("show");
        };
