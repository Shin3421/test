...
<?php include('message.php') ?>
        ...
        <div class="pagetitle">
            ...
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">Customer Task</li>
                </ol>
            </nav>
        </div>
        <section class="section dashboard">
            ...
            <div class="container-fluid px-4">
                    ...
                    <h4 class="text-dark fw-bold lead">
                        Task (Challenges)
                    <a  onclick="add_task()" class="btn text-light float-end" style="background-color:#00c5ce;"><i class="fa fa-add fa-fw" ></i>
                         Add Task
                    </a>
                    </h4>
                </div>
                <!-- content -->
                <div class="card-body mt-4" id="machine_slot">
                <?php
                ...
                ?>  
                    <div class="schedules">
                        ...
                        <h4><i class="fas fa-box"></i> <?php echo $task_name?></h4>
                        <h6><i class="fa fa-list"></i> Description: <?php echo $task_description ?></h6>
                        <h6><i class="fa fa-list"></i> Gain:  <?php echo $task_points ?> Points</h6>
                        <h6><i class="fa fa-list"></i> Target to Acquire:  <?php echo $task_target ?></h6>
                        </div>
                        <div class="schedule-choices">
            <a class="btn btn-success" onclick="edit_task('<?php echo $rows['task_id']; ?>', '<?php echo $rows['task_name']; ?>', '<?php echo $rows['task_description'];?>', '<?php echo $rows['task_points']; ?>', '<?php echo $rows['task_target']; ?>')"><i class="fas fa-edit"></i></a>
            <a class="btn btn-danger" onclick="delete_alert(<?php echo $rows['task_id']; ?>)"><i class="fas fa-trash-can"></i></a>
        </div>
    </div>
    <script type="text/javascript">
        ...
        }).then((result) => 
        {
            ...
            {
                delete_task(<?php echo $rows['task_id'] ?>);
            }
        });
        }
    </script>
    ...
    <!-- /body content -->
    <!-- add modal -->
<div class="modal fade" id="add_task_modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-md">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <input id="add_task_id" name="add_task_id" class="form-control" required type="hidden" value=""/>
        <div class="form-group mt-2" >
        <label>Name</label>
        <input id="add_task_name" name="add_task_name" class="form-control" value="" required/>
        </div>
        <div class="form-group mt-2">
        <label>Description</label>
        <input id="add_task_description" name="add_task_description" class="form-control" value=""/>
        </div>
        <div class="form-group mt-2">
        <label>Points</label>
        <input id="add_add_task_pointstask_points" name="add_task_points" class="form-control" value=""/>
        </div>
        <div class="form-group mt-2">
        <label>Target Progress</label>
        <input id="add_task_target" name="add_task_target" class="form-control" value=""/>
        </div>
        </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
        <button type="submit" class="btn btn-primary" name="add_task_btn">Confirm</button>
      </div>
    </div>
  </div>
</div>
...
<!-- edit modal -->
<div class="modal fade" id="edit_task_modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-md">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <div class="modal-body">
        ...
        <input id="etask_id" name="etask_id" class="form-control" required type="hidden" value=""/>
        <div class="form-group mt-2" >
        <label>Name</label>
        <input id="etask_name" name="etask_name" class="form-control" value="" required/>
        </div>
        <div class="form-group mt-2">
        <label>Description</label>
        <input id="etask_description" name="etask_description" class="form-control" value=""/>
        </div>
        <div class="form-group mt-2">
        <label>Points</label>
        <input id="etask_points" name="etask_points" class="form-control" value=""/>
        </div>
        <div class="form-group mt-2">
        <label>Target Progress</label>
        <input id="etask_target" name="etask_target" class="form-control" value=""/>
        </div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
        <button type="submit" class="btn btn-primary" name="save_task">Save changes</button>
      </div>
    </div>
  </div>
</div>
    ...
    <script type="text/javascript">
        function edit_task(task_id, task_name, task_description, task_points, task_target)
        {
            $('#edit_task_modal').modal("show");
            $('#etask_id').val(task_id);
            $('#etask_name').val(task_name);
            $('#etask_description').val(task_description);
            $('#etask_points').val(task_points);
            $('#etask_target').val(task_target);
        };
            $('#edit_task_form').submit(function(e)
            {
                action = 2;
                e.preventDefault();
                task_id = $('#etask_id').val();
                task_name = $('#etask_name').val();
                task_description = $('#etask_description').val();
                task_points = $('#etask_points').val();
                task_target = $('#etask_target').val();
                $.ajax({
